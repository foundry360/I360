    ...
    <Dialog open={isAssessmentModalOpen} onOpenChange={closeAssessmentModal}>
      <DialogContent className="h-full max-h-[90vh] w-full max-w-[90vw] flex flex-col p-0">
        <DialogHeader className="p-6 pb-0">
          ...
          <DialogDescription>
            {assessmentToResume?.id ? `Resuming assessment: ${assessmentToResume.name}` : (assessmentToResume ? `Rerunning assessment: ${assessmentToResume.name}` : 'Complete the form below to receive an AI-powered analysis of your go-to-market readiness')}
          </DialogDescription>
        </DialogHeader>
